...
def forward(entrada, pesos): # Degrau bipolar
    u = np.dot(entrada, pesos)
    return 1 if u >= 0 else -1  

def atualizar_pesos(entrada, pesos, saida_esperada, saida_calculada, taxa=0.05):
    erro = saida_esperada - saida_calculada
    return pesos + taxa * erro * entrada

# Treinamento
pesos = np.random.uniform(-0.5, 0.5, 4)  
taxa = 0.05  
historico_acuracia = []

for epoca in range(250): #10 = 36.67% ~ 56.67%|| 50 = 63.33% ~ 70%|| 100  = 73.33% ~ 83.33% || 250 = 76.67 ~ 93.33|| 500 = 100% (Na minha maquina)
    acertos = 0
    for i in range(len(X_treino)):
        saida = forward(X_treino[i], pesos)
        pesos = atualizar_pesos(X_treino[i], pesos, y_treino[i], saida, taxa)
        if saida == y_treino[i]:
            acertos += 1
    
    acuracia = acertos / len(X_treino)
    historico_acuracia.append(acuracia)
    
print(f"Resultado final do treino: {historico_acuracia[-1]*100:.2f}%")
